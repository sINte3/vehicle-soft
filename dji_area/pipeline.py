...
def load_flights(con, date_from, date_to, flight_ids=None):
    ...
    for r in rows:
        ...
        # `drone_flights.raw_json` им не является. Прежняя редакция этого
        # комментария обосновывала A13 тем, что колонку «переписывает любой
        # повторный приём списка» -- это НЕВЕРНО и проверено: `api_flight_sync`
        # пропускает дубликаты (`drones.py:578`) и пишет `raw_json` только при
        # вставке (`drones.py:630`), других писателей в приложении нет.
        # Настоящая причина проще и от этого не слабее: колонка НЕ покрыта
        # никаким SHA, её правит любой SQL, миграция или `init_data.py`, и она
        # хранит НАШУ копию строки, а не ответ DJI. Проверить её нечем.
        # Значения списка, разобранные из захешированной ревизии, уже
