...
def context_page_urls(context):
    ...
    [REASON]: причина фильтра НЕ в том, что закрытая страница бросает. Она
    не бросает: `Page.url` -- это `main_frame.url`, то есть `self._url or
    ''`, а закрытие страницы её `_url` не чистит. Закрытая вкладка честно
    возвращает ПОСЛЕДНИЙ известный адрес, и это опаснее исключения. Мёртвая
    вкладка на `/records/list` подтвердила бы вход устаревшей строкой --
    ложное подтверждение, после которого сохраняется сессия закрытого окна.
    Обратный случай мягче: мёртвая вкладка на `/login` просто тянула бы
    ожидание вниз до таймаута.

    [REASON]: закрытая вкладка -- норма, а не сбой: кабинет закрывает попап
    входа сам. Поэтому она выпадает из списка, а не обрывает ожидание.
    """
    urls = []
