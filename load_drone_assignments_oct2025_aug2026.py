...
Ставит операторов на машины за октябрь-2025 .. август-2026 -- 25 назначений,
выведенных из листа v3, который заполнили диспетчеры и принял владелец
...
ДО ЭТОГО ЗАГРУЗЧИКА В БАЗЕ УЖЕ ЛЕЖАЛИ ДВЕ РУКОТВОРНЫЕ СТРОКИ -- id=1 (Усмон,
№4, 01.04-13.04.2026) и id=2 (Усмон, №8, с 14.04.2026). Это граница 13/14
апреля из сверки 2026-08-03, записанная в докстринге DroneOperatorAssignment.
Сухой прогон 2026-08-15 их нашёл; загрузчик их НЕ трогает и обходит встык:
на проде вставляется 23 строки, две пропускаются как уже лежащие.
...
ASSIGNMENTS = (
    ...
    # [REASON]: строка v3 по машине 4 (01.09.2025-13.04.2026) разрезана на
    # две ПО ЖИВОМУ ПРОГОНУ 2026-08-15. На проде уже лежит рукотворная строка
    # id=1 (Усмон, №4, 01.04-13.04.2026) -- это граница 13/14 апреля из
    # сверки 2026-08-03, записанная в докстринге DroneOperatorAssignment.
    # Цельная строка пересеклась бы с ней и была бы пропущена ЦЕЛИКОМ --
    # первый сухой прогон ровно так и сделал, теряя октябрь-март (~230 га).
    # Поэтому: дополнение до рукотворной строки плюс её дубль для пустой
    # базы, встык 31.03 -> 01.04, без нахлёста.
    (4, 'Анваров Усмон', '2025-10-01', '2026-03-31', SHEET,
     V3_NOTE + '; начало 01.09 обрезано до 01.10 -- сентябрь загружен; '
     'конец 31.03 -- дополнение ВПРИТЫК к рукотворной строке id=1 '
     '(01.04-13.04, граница сверки 2026-08-03), чтобы не пересечься с ней'),
    (4, 'Анваров Усмон', '2026-04-01', '2026-04-13', SHEET,
     V3_NOTE + '; на проде уже лежит рукотворной строкой id=1 и там будет '
     'пропущена гвардией -- строка здесь для полноты листа на чистой базе'),
    ...
    (8, 'Анваров Усмон', '2026-04-14', None, SHEET,
     V3_NOTE + '; на проде уже лежит рукотворной строкой id=2 (граница '
     'сверки 2026-08-03) слово в слово и там будет пропущена гвардией'),
