...
ОГРАНИЧЕНИЕ МАНИФЕСТА. ``exporter_sha256`` и ``coverage_module_sha256`` --
хеши файлов НА ДИСКЕ. На сервере рабочая копия Windows несёт CRLF через
``core.autocrlf``, поэтому напрямую с Linux-копией они не совпадут; сверять
надо после перевода LF -> CRLF. Проверено на прогоне 11.09.2026: после
перевода совпало байт в байт. ``repo_commit`` рядом называет коммит, и он
единственный, кто идентифицирует код однозначно.
