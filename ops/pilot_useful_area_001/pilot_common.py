...
ревизии продукта ещё нет, поэтому скрипт, требующий `HEAD == PRODUCT_SHA` от
репозитория, в котором сам лежит, требовал собственного отсутствия.

Ревизий две, и у них разные роли:

* **PRODUCT_SHA** -- проверенная ревизия ПРОДУКТА. С 2026-09-05 это
  `a5dc4e6` -- коммит продукта `useful-area-v2` внутри того же PR, что и
  этот комплект (первый пилот 2026-09-04 стоял на `c3e6a12`, merge PR #113,
  и показал, что правила v1 негодны). На ней стоит площадка, из неё
  материализуются миграция, `migration_utils`, инструмент бэкапа и
  инструмент пересчёта;
