...
* список намеренных различий (`kit_differs_on_purpose`) снова НЕ пуст: с
  2026-09-09 в нём `drone_collector/main.py`.

История этой записи -- ровно то, ради чего механизм и оставлен. Пока
площадка стояла на `c3e6a12`, `main.py` различался намеренно: ревизия
комплекта добавила в сводку сбора два числа (`probe_request_failures`,
`probe_pending_requests`). Ревизия продукта `useful-area-v2` это изменение
уже содержала, и 2026-09-05 список опустел. Инкремент
`DJI-AREA-EVIDENCE-001` снова развёл ревизии: сборщик получил режимы
`--sources`, `--send-sources`, `--land-snapshot`, `--send-snapshot` и коды
выхода 18/19/20, которых на одобренной ревизии пилота `PRODUCT_SHA` нет и
быть не может.

[REASON]: это ОБЪЯВЛЕННОЕ расхождение после пилота, а не поблажка ради
зелёного CI. `PRODUCT_SHA` не двигается, `product_blob` по-прежнему несёт
байты одобренной ревизии, и `materialize()` для файлов этого списка берёт
именно `product_blob` -- старый пилот воспроизводится на своей ревизии в
точности как прежде. Сила гейта сохранена с другой стороны: `kit_blob`
приколот к нынешним байтам, поэтому СЛУЧАЙНАЯ правка `main.py` по-прежнему
ловится как `WORKTREE_DIFFERS`, а его пропажа -- как `MISSING`. Разошлись
ревизии -- это записывается здесь; молча совпадающими они не считаются.
...
# Файлы, которые у комплекта СВОИ и отличаются от продукта осознанно.
#
# `drone_collector/main.py` -- расхождение после пилота: `DJI-AREA-EVIDENCE-001`
# добавил сборщику режимы захвата источников и коды выхода 18/19/20, которых на
# одобренной ревизии пилота нет. Пилот от этого не ломается: он свой `main.py`
# не материализует и не исполняет (материализуются только миграция,
# `migration_utils.py` и `backup_transport_db.py`), а сборщик на BAK-TEX11 и
# так работает на ревизии КОМПЛЕКТА -- роль `collector` сверяет этот файл с
# `kit_blob`, как и должна.
KIT_ONLY_FILES = (
...
def check_against_worktree(manifest, repo=REPO_ROOT):
    ...
    for path, entry in sorted(manifest['kit_differs_on_purpose'].items()):
        ...
        full = os.path.join(repo, path.replace('/', os.sep))
        # [REASON]: пропажа файла обязана называться пропажей, как и в списке
        # одинаковых. Пока список намеренных различий был пуст, `if exists`
        # ничего не стоил; с появлением в нём `drone_collector/main.py` --
        # файла, который сборщик РЕАЛЬНО исполняет, -- молчание об исчезнувшем
        # файле стало бы ослаблением гейта ровно там, где он нужен.
        if not os.path.exists(full):
            problems.append('MISSING:%s' % path)
            continue
        if common.worktree_blob_sha(repo, path) != entry['kit_blob']:
