...
## Порядок работы сессии — `docs/AGENT_SESSION.md`

Разработка ведётся облачными сессиями Claude Code. **Прочитать
`docs/AGENT_SESSION.md` сразу после этого файла и до трекового.** Там: где
сессия находится и чего из неё не достать, что она решает сама и чего не имеет
права выдумать, чем доказывает сделанное, как оформляет ветку и PR.

Три факта, которые устав задаёт, а порядок работы объясняет:

- **Сессия работает в одноразовом Linux-контейнере.** Staging и production из
  неё недостижимы; команды PowerShell в этом файле и в ранбуках — для
  владельца, не для агента. Работа сессии заканчивается смерженным PR.
- **Спрашивать не надо там, где можно решить.** Технические выборы делаются
  самостоятельно и называются в описании PR. Неоднозначность задания толкуется
  как внимательным коллегой, с объявленным допущением.
- **Бизнес-правила по-прежнему выдумывать нельзя.** Но это не повод встать:
  делается всё, что от неизвестного правила не зависит, недостающее остаётся
  незакрытым, а вопрос пишется в описание PR и в «Что открыто» трекового файла.

Среда поднимается сама: `.claude/hooks/session-start.sh` ставит зависимости и
печатает состояние гейта, площадки и прода в первых строках сессии.
