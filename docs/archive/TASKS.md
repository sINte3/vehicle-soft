> **АРХИВ. Новые записи сюда не добавляются.** Текущее состояние площадок и журнал релизов — в `docs/DEPLOYED.md`.
> Занятость staging — в `docs/STAGING.md`. Работа трека — в `docs/tracks/<трек>.md`.
> Шапка этого файла устарела и больше не обновляется — не верьте ей.
