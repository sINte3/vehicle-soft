...
#### Предсказание на первый релиз после `0bbfcbf`, записанное заранее

Мерж в `main` ничего не кладёт на сервер, поэтому `TOOL-DRIFT-LEGACY-001`
подтверждён пока **только на фикстуре**, а подтвердить его на живом реестре
можно ровно один раз — на следующем прод-деплое, когда старый чекер сменится
новым; после этого базовая точка исчезнет навсегда.

**База, снята на production 2026-08-03 при `HEAD` = `292c53e`** — то есть
прежним чекером, до `0bbfcbf`:

| величина | значение |
|---|---|
| `registered-but-no-file` | 15 |
| `unclassifiable` | 21 |
| `file-but-not-registered` | 0 |
| записей в реестре | 32 |
| файлов `migrate_*.py` | 38 (17 с `MIGRATION_ID`) |
| `SyntaxWarning` от `migrate_add_wialon.py` | **присутствует** |

**Чем обязан ответить тот же прогон после релиза, привёзшего `0bbfcbf`:**

| величина | обязано стать |
|---|---|
| `resolved-by-backfill` | 9 |
| `registered-but-no-file` | 15 → **6** |
| `unclassifiable` | 21 → **12** |
| `file-but-not-registered` | 0 (без изменений) |
| `SyntaxWarning` | **не печатается** |

**Отсутствие `SyntaxWarning` — самый быстрый признак того, что отработал новый
чекер, а не прежний**: warning виден в первой же строке вывода, до всяких
чисел, и ни с чем не путается.

Код возврата останется **1**: шесть имён и двенадцать файлов карта бэкфилла не
разрешает, и это ожидаемый результат, а не провал прогона. Ненулевой код здесь
означает «хвост на месте», а не «дрейф найден» — дрейф показывает только
`file-but-not-registered`.

Строки в `RELEASE_GATE.md` под этот пункт **нет намеренно**: проверить его
можно исключительно прод-деплоем, а строка гейта прод-деплой как раз
запрещает — вышел бы тупик.
