...
Если в выводе есть строка `WARNING: N flight(s) failed coverage` — это **не
остановка**. Сбойный вылет не имеет права стоить всей поездки на сервер,
поэтому он помечается в `coverage.csv` статусом `FAILED` с текстом ошибки, а
остальные считаются. Блок доводится до конца, архив присылается как есть, и
такие строки разбираются здесь.
