## Текущее состояние — снимок на 2026-07-17 (обновлять при каждой вехе)
...
- Staging / origin/main: commit `7d7c1f4` (PR #11 SP-DESK-001 смержен,
  `b8cfc3e → 7d7c1f4`). Включает весь готовый модуль «Запчасти» + инкремент 1
  заимствования (рабочий стол оператора). Развёрнут на staging (git pull +
  рестарт `TransportReportStaging`).
- Активная задача: **внедрение функционала программ-аналогов В СУЩЕСТВУЮЩИЙ
  ...
  по своему усмотрению. Следующий шаг — **инкремент 2: редизайн карточки
