## Текущее состояние — снимок на 2026-07-12 (обновлять при каждой вехе)
...
- Дата фиксации: 2026-07-12 (вечер).
- Production: commit `a60eb0e`. **Этап 1 и Этап 2 модуля «Запчасти» ОБА
  развёрнуты и работают на production.** Деплой Этапа 2 выполнен 2026-07-12
  вечером (низкая активность): бэкап → `git pull --ff-only` (`0de0027` →
  `a60eb0e`) → шрифты проверены → `pip install -r requirements.txt`
  (`reportlab==5.0.0` уже стоял) → `migrate_spare_parts_stage2.py` (успешно) →
  рестарт `TransportReport` → смоук-тест на живом production агентом (все 4
  пункта PASS: SKU-путь целиком, акт на RU и UZ, «Основание» в журнале,
  ненулевая сумма в отчётах с учётом issued-заявок) → тестовые данные убраны
  (остаток склада обнулён корректировкой, SKU «SMOKE-TEST» деактивирован,
  пароль admin сменён на нестандартный после того, как агент был вынужден
  его сбросить). Подробности — «Модуль запчастей → Этап 2» ниже.
- Staging / origin/main: тот же коммит `a60eb0e`, staging и production
  синхронизированы.
- Активная задача: модуль «Запчасти» Этапы 1-2 полностью закрыты. Следующий
  шаг по дорожной карте — MVP «Заявки/Наряды» (mobile-first, без GPS).
- Незакрытые риски: ни одного блокирующего.
  - Правило 4 (аномалия расходов по категории) не прогонялось отдельным живым
    тестом — тот же код-паттерн, что и подтверждённые правило 3 и отчёты,
    риск низкий.
  - Косметика (бэклог полировки, не блокирует): склад организации «Когон ПТЗ»
    на production называется «temp» (создан агентом при смоук-тесте под этим
    именем) — интерфейс не даёт переименовать существующий склад; на расчёты
    не влияет. `datetime.utcnow()` deprecation warning в
    `migrate_spare_parts_stage2.py` (Python 3.14) — не блокирует, техдолг.
  - На production в истории теперь есть один тестовый акт `SPW-2026-00001`
    (заявка #4, «Головка блока цилиндров», SKU SMOKE-TEST, деактивирован) —
    неудаляем по дизайну («acts are permanent»), явно помечен в примечании
    заявки как smoke-test, известен и задокументирован здесь.
  - На staging в рабочей копии есть untracked-файлы (промпты `.md` и
    диагностические скрипты `inspect_*.py`) — при коммитах исключать их явно
    (`git add <конкретный файл>`, НЕ `git add -A`).

## Запчасти — Этап 3 (совместимость) — 🟡 РАЗРАБОТАН, PR открыт, НЕ развёрнут

- Дата: 2026-07-12 (ночная сессия, cloud Claude Code). Ветка
  `claude/new-session-0t8kdr`. Разработано, протестировано в песочнице,
  запушено, открыт PR. **Не смёржено и НЕ развёрнуто** — ждёт ревью
  владельца/архитектора (как Этап 2). Доступа к живым серверам у этой сессии
  нет по построению.
- Что сделано (аддитивно, ни одна колонка не удалена/переименована):
  1. Справочник моделей техники `equipment_models` + аддитивная колонка
     `equipment.model_id` (nullable). Миграция `migrate_spare_parts_stage3.py`
     заполняет по одной модели на каждое различное непустое `eq_type` (без
     объединения похожих — это ручное решение), проставляет `model_id`. `eq_type`
     НЕ трогается — все старые читатели (`daily_entry.html`, форма техники,
     `init_data.py`) работают как раньше. Идемпотентна.
  2. Экран управления моделями (`/spare-parts/equipment-models`): список со
     счётчиками техники/совместимости, переименование (синхронизирует `eq_type`
     на связанной технике), объединение (переносит технику, совместимость и
     нормы на «приёмник», деактивирует, а не удаляет). Это инструмент для
     ручной чистки ~336 моделей — делается владельцем после деплоя, в своём
     темпе.
  3. Пикер модели на форме техники (`ref_equipment.html`, add + edit) с опцией
     «➕ новая модель»; проставляет `model_id` И `eq_type` (текст = имя модели).
     Пустой пикер → старое поведение free-text `eq_type` (обратная совместимость).
  4. Матрица совместимости `spare_part_compatibility` (UNIQUE part+model) +
     редактор на каждой детали каталога (кнопка 🔗). **Отсутствие строк = «не
     задано», НЕ «несовместимо со всем»** — правило 5 молчит для таких деталей.
  5. Правило 5 (несовместимость), severity **RED** как правила 1-2, warn-only.
     Срабатывает только если у детали есть строки совместимости И модель техники
     не входит в них. Молчит при пустой матрице или NULL `model_id`. Встроено в
     тот же `_check_extra_warnings` → и AJAX формы, и блок детали (шаблон детали
     поправлен, чтобы rule 5 рендерился красным).
  6. Нормы моточасов `spare_part_maintenance_norms` + CRUD
     (`/spare-parts/maintenance-norms`) + пассивный список «Пора менять»
     (`/spare-parts/maintenance`). Авто-заявок НЕ создаётся (отложено владельцем).
- **Решение по Части 4, требует подтверждения ревьюера** (архитектор не имел
  кода Wialon при написании ТЗ): `EngineHoursRecord.engine_hours` — ПОДНЕВНОЕ
  значение, не кумулятивный одометр; снимка моточасов на момент выдачи детали
  НЕТ нигде. «Наработка с последней замены» вычисляется как SUM подневных
  моточасов ПОСЛЕ даты последней выданной write-off-акта детали на этой технике.
  Молчит, если нет якоря (деталь ни разу не выдавалась через модуль) или нет
  данных моточасов → может только НЕ-досработать (пропустить), но не даёт ложных
  срабатываний. Подробности и обоснование — в теле PR.
- Разрешения: все новые экраны под существующим `spare_parts_catalog_manage`,
  новых app_modules не добавлено.
- Тесты (в песочнице, temp SQLite): миграция (различные/общие/near-match/пустые
  `eq_type`, идемпотентность, UNIQUE), правило 5 (обе стороны + оба «молчит»),
  наработка (досработал/не досработал/никогда-не-выдавался), объединение моделей
  (перенос+синхронизация+дедуп), форма техники (existing/inline-new/legacy),
  синхронизация `eq_type` при переименовании, редактор совместимости, рендер
  всех новых экранов (200). Все PASS.
...
Справочник моделей техники (сейчас `Equipment.eq_type` — свободный текст),
матрица совместимости `spare_part_compatibility`, warning-правило 5
(несовместимость), нормы моточасов из Wialon → автопредложение заявки. Точный
объём согласовать с владельцем перед стартом.
