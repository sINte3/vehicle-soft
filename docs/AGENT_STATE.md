## Текущее состояние — снимок на 2026-07-11 (обновлять при каждой вехе)
...
- Production: commit `6d797f3` (merge PR #3, «spare-parts-mobile-upload-and-i18n-fixes»).
  Модуль запчастей Этап 1 полностью развёрнут и работает на production.
- Staging / origin/main: commit `e4f7447` — merge PR #4 («spare-parts-stage2»:
  warning rules 3/4/6, fuzzy search, SKU catalog, warehouse inventory, write-off
  acts PDF). Подтверждено `git log -1` на staging 2026-07-11
  (`e4f7447c94054124aa7dc42ada1045c207f6b144`, merge of `6d797f3` + `171a92c`).
- Активная задача: главный вопрос QA Этапа 2 (SKU → заявка → выдача →
  списание) ЗАКРЫТ и подтверждён на живых данных 2026-07-11 (заявка #10,
  остаток 5→4). Осталось: акт на узбекском на живом сервере, срабатывание
  правила 3, решить судьбу мелкой находки по подписи в журнале движений
  склада (см. «Модуль запчастей — детальный статус» ниже) — затем деплой
  Этапа 2 на production.
- Незакрытые риски: см. «Модуль запчастей → Этап 2» ниже (мелкая находка —
  «Основание» в журнале движений склада показывает ID позиции заявки вместо
  номера заявки; не искажает акт/остатки, только подпись в журнале).
  Также на staging в рабочей копии есть untracked-файлы (промпты `.md` и
  диагностические скрипты `inspect_*.py`) — при коммите дока исключить их явно
  (`git add docs/AGENT_STATE.md`, НЕ `git add -A`).
...
### Этап 2 — 🟡 PR #4 смёржен, на staging, идёт QA
...
**QA на staging — статус на 2026-07-11:**
...
- ✅ Выдача заявки формирует акт (транзакция отрабатывает)
- ✅ Выдача БЕЗ SKU подтверждена (заявка #9, 2026-07-11): корректно показала
  предупреждение и потребовала явный чекбокс, акт `SPW-2026-00001` помечен
  «Позиции без SKU выданы без списания со складского учёта», остаток склада
  не тронут.
- ✅ ОТКРЫТЫЙ ВОПРОС ПОЛНОСТЬЮ ЗАКРЫТ (2026-07-11) — путь SKU → заявка →
  выдача → списание пройден целиком на живых данных, не только по коду:
  приход SKU «Varta / 4893 / Bazar» (Аккумуляторная батарея) на склад
  «Когон ПТЗ» → 5.0; новая заявка #10 — деталь выбрана из каталога, SKU-
  дропдаун появился и SKU выбран; утверждена; на экране выдачи показана
  таблица «Будут списаны со склада» (не предупреждение, как в ветке без
  SKU) с текущим остатком 5.0; после выдачи — акт `SPW-2026-00002` с
  колонкой SKU заполненной; остаток склада стал 4.0; движение в журнале —
  «Выдача −1.0, остаток после 4.0». Число сошлось (5 − 1 = 4). SKU-дропдаун
  подтверждённо показывается, когда у канонической детали есть хотя бы один
  активный SKU — гипотеза (б) из предыдущей сессии верна в обе стороны.
- ⚠️ НОВАЯ МЕЛКАЯ НАХОДКА (не блокирует, но стоит поправить перед проду):
  в «Склад → Последние движения» колонка «Основание» для выдачи по SKU
  показывает «позиция заявки #11» — это `SparePartRequestItem.id` (строка
  1767–1827, `_apply_inventory_movement`/`issue_request` вызывает её с
  `reference_id=item.id`), а НЕ номер заявки (была заявка #10). Само
  движение и акт данные не искажают (акт правильно показывает «Заявка: #10»,
  сумма и остаток верны) — искажена только подпись в журнале движений
  склада, что может сбить с толку при аудите. При этом поле `note` в БД
  заполняется корректно (`'Request #{}'.format(spr.id)` → «Request #10») —
  похоже, шаблон `spare_parts_inventory.html` строит подпись из
  `reference_id`, а не читает готовый `note`. Кандидат на однострочный фикс:
  показывать `movement.note` вместо реконструкции по `reference_id`. Не
  проверял сам шаблон построчно — нужен файл `spare_parts_inventory.html`,
  если решим чинить сейчас, а не в бэклоге.
- ⬜ Не проверено: акт на узбекском на живом сервере, срабатывание правила 3.

**Чтобы считать Этап 2 завершённым:**

1. Пройти сценарий «SKU у детали → заявка → выдача → списание» целиком на
   staging — ГОТОВО, подтверждено 2026-07-11 (заявка #10, см. выше).
2. Пройти оставшиеся пункты QA (⬜ выше: УЗ-акт, правило 3).
3. Решить: чинить находку по «Основание» в журнале движений сейчас (мелкий
   PR) или отправить в бэклог полировки — не блокирует деплой.
4. Деплой на production: бэкап → `git pull` → проверить шрифты в
   `static/fonts/` (не 0 байт) → `pip install -r requirements.txt` →
   `migrate_spare_parts_stage2.py` → старт сервиса → выдать права реальным
   людям (`inventory_manage` завскладу, `issue` кладовщику) → smoke-тест.
   (Только эта одна миграция — Этап 1 и сид каталога уже применены ранее.)
