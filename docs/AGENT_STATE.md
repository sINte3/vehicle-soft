...
- Staging / origin/main: тот же коммит `ed8ca9c`, staging и production
  синхронизированы.
- Активная задача: **весь модуль «Запчасти» закрыт, включая полный
  регрессионный прогон по всем трём этапам (staging, 2026-07-13, два захода
  агентом — второй по строгим правилам «только через браузер, никакого
  прямого доступа к БД/сервисам»).** Все шесть warning-правил (1-6) теперь
  подтверждены живьём хотя бы раз — включая правила 4 и 6, которые не
  проверялись вживую с самого начала работы над Этапом 2. Скриншоты правил 4
  и 6 лично сверены архитектором дословно с текстом в отчёте агента. Следующий
...
## 2026-07-17 — SP-DESK-001 реализован (ветка claude/session-wq29yw, ждёт staging QA)
