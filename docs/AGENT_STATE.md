...
- Production: commit `6d797f3` (merge PR #3, «spare-parts-mobile-upload-and-i18n-fixes»).
  Модуль запчастей Этап 1 работает на production. Этап 2 туда ещё НЕ развёрнут.
- Staging / origin/main: commit `c4ee7ef` — PR #4 (Этап 2, commit `e4f7447`) +
  консолидированный QA-fix PR (`claude/new-session-4eqh0f`, fast-forward merge
  2026-07-12) поверх него. Все три QA-находки Этапа 2 исправлены и подтверждены
  живыми тестами на staging (см. «Модуль запчастей → Этап 2» ниже).
- Активная задача: **Этап 2 полностью закрыт на staging, готов к деплою на
  production.** Следующий шаг — сам деплой (чек-лист ниже), затем — MVP
  «Заявки/Наряды» по дорожной карте.
- Незакрытые риски: ни одного блокирующего. Необязательный довесок: правило 4
  (аномалия расходов по категории) не прогонялось живым тестом отдельно —
  тот же код-паттерн, что и в правиле 3 и в отчётах (оба подтверждены живьём),
  риск низкий, но если нужна полная уверенность — прогнать сценарий на staging
  перед деплоем. На staging в рабочей копии есть untracked-файлы (промпты `.md`
  и диагностические скрипты `inspect_*.py`) — при коммитах исключать их явно
...
### Этап 2 — ✅ ЗАВЕРШЁН на staging, готов к деплою на production (PR #4 + QA-fix PR)
...
**Этап 2 закрыт. Чек-лист деплоя на production:**

1. Бэкап продовой БД (`backup_production_db.bat` / стандартная процедура).
2. `git pull --ff-only` на `C:\transport-report` (production) — подтянет и
   PR #4, и QA-fix PR разом, т.к. на staging они уже слиты в `main`.
3. Проверить шрифты в `static/fonts/` на production (не 0 байт) —
   `DejaVuSans.ttf`, `DejaVuSans-Bold.ttf`.
4. `pip install -r requirements.txt` (новая зависимость `reportlab`).
5. Прогнать `migrate_spare_parts_stage2.py` (идемпотентна; Этап 1 и сид
   каталога уже применены ранее, повторно не нужно).
6. Перезапустить сервис `TransportReport` (NSSM).
7. Выдать права реальным людям: `spare_parts_inventory_manage` — завскладу,
   `spare_parts_issue` — кладовщику.
8. Smoke-тест на production: одна тестовая заявка с SKU от начала до выдачи,
   акт на обоих языках, отчёты показывают ненулевую сумму.
9. Обновить этот файл финальной записью о деплое на production (дата,
   commit hash) — как обычно после production-релиза.
