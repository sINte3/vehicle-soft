...
Среда разработки — без production-базы: числа приёмки QA-раунда измерены
на синтетической базе (это сказано в каждом коммит-сообщении); повторная
проверка 41–50 на реальной базе — открытый остаток в RELEASE_GATE.
