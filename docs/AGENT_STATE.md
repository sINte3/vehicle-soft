...
- Production: commit `a60eb0e`. Этап 1 и Этап 2 модуля «Запчасти» работают на
  production. **Этап 3 (справочник моделей техники, совместимость, правило 5,
  моточасы) туда ещё НЕ развёрнут** — только на staging, готов к деплою.
- Staging / origin/main: commit `0c9b1c6` (PR #5, ветка
  `claude/new-session-0t8kdr`, смёржена владельцем на GitHub 2026-07-13).
  Развёрнуто на staging 2026-07-13: сервис остановлен →
  `git pull --ff-only` (`a60eb0e` → `0c9b1c6`) → `migrate_spare_parts_stage3.py`
  (55 моделей создано автоматически из 55 различных значений `eq_type`, все
  336 единиц техники привязаны) → сервис запущен. Полная визуальная QA
  живым агентом — все 6 сценариев PASS (см. «Модуль запчастей → Этап 3» ниже).
- Активная задача: Этап 3 полностью проверен на staging, готов к деплою на
  production по тому же чек-листу, что и Этап 2 (см. ниже). После —
  MVP «Заявки/Наряды» по дорожной карте.
- Незакрытые риски: ни одного блокирующего для деплоя.
  - Правило 4 (аномалия расходов, Этап 2) по-прежнему не прогонялось отдельным
    ...
    отчёты, риск низкий, актуально с прошлой записи.
  - В справочнике моделей (Этап 3, автомиграция) есть ожидаемые дубли/неполные
    записи из грязных исходных `eq_type` (пример: отдельная запись
    «Экскаватор» без марки/модели, count=1, рядом с более полными
    «HYUNDAI Экскаватор 140 w-95» и др.) — это НЕ баг, это и есть то самое
    «разовая сверка», ради которой сделан экран объединения моделей;
    исправляется владельцем в своём темпе через `/spare-parts/equipment-models`,
    не блокирует ничего.
  - Косметика (бэклог полировки, не блокирует): склад организации «Когон ПТЗ»
    на production называется «temp» (см. предыдущую запись). `datetime.utcnow()`
    deprecation warning в обеих Stage-миграциях (Python 3.14) — техдолг.
  - На production есть один тестовый акт `SPW-2026-00001` (задокументировано
    ранее, неудаляем по дизайну, явно помечен как smoke-test).
...
### Этап 3 — ✅ ЗАВЕРШЁН на staging, готов к деплою на production (PR #5)
...
**Чтобы считать Этап 3 завершённым — деплой на production:**
Тот же чек-лист, что и для Этапа 2 (см. выше): бэкап → `git pull --ff-only`
на `C:\transport-report` → (новых зависимостей нет, `pip install` можно
пропустить или прогнать для подстраховки) → `migrate_spare_parts_stage3.py`
→ рестарт `TransportReport` → smoke-тест → обновить этот файл финальной
записью.
