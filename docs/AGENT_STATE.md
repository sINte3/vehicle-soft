...
- Production: **`436d8ae`** — коммит доков трека АЗС поверх `402abb7`
  ...
  **ВНИМАНИЕ, трек Запчастей.** Вместе с деплоем трека АЗС на production
  ...
  единица `kv_metr`, и при необходимости перенести и выполнить скрипт —
  задача трека Запчастей, здесь только зафиксирована.
...
## 2026-07-23 — SP-REPORTS-007 (инкремент 7): разделение экрана отчётов, выгрузки Excel и PDF, переименование «SKU» → «Артикул» — PR #17, #18, #19, развёрнут на staging
...
### Хвосты вехи (перенести в следующий чат)

Из 37 пунктов чек-листа `RUNBOOK_INC7_STAGING.md` закрыто около двенадцати.
Не проверено:

- сохранение всех пяти фильтров после submit и перезаполнение списка техники
  при смене организации на страницах отчётов;
- 403 для пользователя без права `spare_parts_reports`;
- 404 на `/spare-parts/reports/nonsense` и на удалённом `/spare-parts/reports/export`;
- пустой период (будущие даты) → пустое состояние, а не ошибка;
- секция D целиком: пункт меню «Артикул», экран `/spare-parts/skus` после
  `FIELD-CATNO-001`, блок про позиции без артикула в карточке заявки,
  «Артикул: …» в PDF акта, колонка «Артикул» в Excel очереди закупки;
- секция E целиком: создание и редактирование артикула с ошибкой дубля,
  очередь закупки и её экспорт, движения склада, минимумы, генерация акта
  при выдаче;
- **PR #19 на staging не проверялся вообще**: десять PDF на предмет переносов
  в шапках и лаунчер на трёх ширинах (3+2 / 2+2+1 / одна колонка).

Прочие хвосты:

- `add_unit_sqm.py --apply` на production не выполнен. Без него ввод остатков
  по пилоту 1С упрётся в отсутствующую единицу «кв. метр».
- На staging лежат записи техники `CODEX-SOL-SPARE-20260713-140035` и
  ...
  **техника**, а не заявки, и маркер `DEMO-2026H1` у `seed_demo_orm.py
  --cleanup` их не захватит. Список чистки перед прод-деплоем расширить.
- `git config --global user.name` / `user.email` на сервере так и не заданы.
