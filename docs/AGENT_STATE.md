...
раз в этом треке в правильном порядке, поэтому строка в `docs/RELEASE_GATE.md`
не заводилась: непровалидированного в `main` не было ни минуты. Дерево
мерж-коммита совпадает с деревом ветки: `git log --oneline HEAD..origin/main`
перед мержем вернул пустоту, `main` стоял на `d788674`.
