## Текущее состояние — снимок на 2026-07-21 (обновлять при каждой вехе)
...
- Production: commit `20eb172`, тег `v1.1-production-2026-07-21`. Бандл
  PR #8–#15 развёрнут 2026-07-21 (см. датированную запись ниже). Прод и
  staging совпадают по коду; `origin/main` уходит вперёд ровно на коммит
  с этими доками. Применены шесть миграций: `SPARE_PARTS_UNITS`,
  `SPARE_PARTS_ACTS_PERMISSION`, `SPARE_PARTS_SKU_UNIQUENESS`,
  `SPARE_PARTS_NAME_UZ`, `SPARE_PARTS_RESERVATIONS`,
  `SPARE_PARTS_MIN_LEVELS` — реестр `schema_migrations` вырос с 20 записей
  до 26.
- origin/main: commit `20eb172` (доки инкремента 5) плюс коммит с доками
  прод-деплоя от 2026-07-21. Ниже по убыванию: `5157b1c` (PR #15,
...
- Staging: на `main` = `20eb172` (+ коммит доков), развёрнуты инкременты
  1–5, по коду совпадает с продом. Бэкап перед инкрементом 4 лежит в
...
- Инкременты трека, все пять закрыты и лежат в `main`:
...
- Следующий шаг трека **не определён**. Инкременты 1–5 закрыты; какой будет
  шестой, решает владелец. Кандидаты из накопленного бэклога, не
  приоритизированы: снятие резерва при отклонении утверждённой заявки, история
  цен на экране SKU, массовое утверждение заявок. Не скоупить, пока владелец
  не выберет.
