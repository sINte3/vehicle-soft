...
- Дата фиксации: 2026-07-25 (релиз PR #25; закрытие валидации инкремента 7
  трека Запчастей; ранее — закрытие трека АЗС, инкременты A, A2, A3, B, C).
- Production: **`0b74e3f`**. Развёрнутый релиз — **`f085c46`** (мерж PR #25,
  исправление `tojson` в атрибутах: три шаблона, кавычки); поверх него только
  коммиты доков. Тег на этот релиз — **`v1.5-production-2026-07-25`**,
  ...
  **ВНИМАНИЕ, трек Запчастей.** Вместе с деплоем трека АЗС на production
  ...
  существует только в рабочей копии staging). Проверить, есть ли на проде
...
- origin/main: **`10a2847`**. Сверху вниз: `10a2847` (мерж PR #26, трек АЗС —
...
- Staging: `main` = **`10a2847`**, совпадает с `origin/main`. Production —
  `0b74e3f`: отстаёт на мерж PR #26, его коммит и три коммита доков, то есть
  расхождение теперь и по коду тоже. На staging применён
