## Текущее состояние — снимок на 2026-07-18 (обновлять при каждой вехе)
...
- Production: commit `ed8ca9c` — **прод НАМЕРЕННО не трогаем.** Весь цикл
  заимствования у программ-аналогов копится на staging; единый прод-деплой
  бандлом (с бэкапом и в окно низкой активности) — позже, после серии
  инкрементов. На проде: модуль «Запчасти» Этапы 1–3 + два раунда аудита
  (всё развёрнуто ранее, 2026-07-13).
- Staging: развёрнута ветка `sp-detail-002-request-card` (HEAD `321d022`,
  PR #12 — инкремент 2, редизайн карточки заявки). Переключение выполнено
  `git checkout` + `Restart-Service TransportReportStaging`; миграций и
  бэкапа не требовалось (схема не менялась). Самопроверка владельцем по всем
  шести статусам пройдена — браузерный QA-агент в этом инкременте намеренно
  не запускался. Ниже ветки на staging лежит весь готовый модуль «Запчасти»
  + инкремент 1 заимствования (рабочий стол оператора, SP-DESK-001).
- origin/main: commit `ab5c5b2`. **Расхождение с этим файлом:** после
  `7d7c1f4` (мерж PR #11) main продвинулся как минимум двумя
  незадокументированными здесь коммитами (`7d7c1f4` → … → `5771328` →
  `ab5c5b2`). Восстановить состав через `git log --oneline 7d7c1f4..origin/main`
  и дописать при следующей вехе; до этого содержимое `main` считать
  неописанным в AGENT_STATE (авторитетен git log).
- Активная задача: **внедрение функционала программ-аналогов В СУЩЕСТВУЮЩИЙ
  модуль «Запчасти»** (АдептІС / HubEx / QuintaDB / 1С — берём идеи по
  запчастям, НЕ интеграции). Это НЕ отдельный модуль «Наряды» — «Наряды»
  отдельным контуром в этом треке не делаем. Инкремент 1 (рабочий стол
  оператора + ролевые экраны, SP-DESK-001) реализован и рантайм-валидирован
  браузерным QA на staging; правок кода не требуется (детали и дорожная карта —
  в датированной записи 2026-07-17 ниже). Роли пользователям раздаёт владелец
  по своему усмотрению. Инкремент 2 (**SP-DETAIL-002**, редизайн карточки
  заявки: «Следующее действие» + шкала статуса + единая лента событий)
  реализован, развёрнут на staging и проверен владельцем — подробности в
  датированной записи 2026-07-18 ниже; мерж PR #12 в `main` на момент
  фиксации ещё не выполнен. Следующий шаг — **инкремент 3: резерв и
  доступный остаток** (три вида: На складе / Зарезервировано / Доступно =
  склад − резерв; авторезерв при утверждении, освобождение при отмене) —
  трогает склад, потребует миграцию схемы.
- Незакрытые риски: ни одного блокирующего.
  - **Новая находка регрессионного прогона (некритично, бэклог полировки):**
    позиция заявки с количеством 0 не сохраняется — на поле стоит HTML5
    `min="0.001"`, браузер молча блокирует отправку формы, видимого сообщения
    об ошибке нет (JS не показывает нативную валидацию). Серверная проверка,
    судя по всему, не достигается вообще. Рекомендация: добавить
    `reportValidity()`/явное сообщение на фронте, оставить сервер как
    страховку.
  - Дубли SKU разрешены без предупреждения (нет уникальности по
    деталь+бренд+артикул+поставщик) — не обязательно баг (SKU может
    отличаться партией), но не задокументировано как осознанное решение;
    решить, стоит ли добавлять предупреждение в UI.
  ...
  - На production есть тестовые черновики заявок #5 и #6 (Этап 3) и #4 (Этап 2,
    ...
    черновики не участвуют в отчётах/остатках, оставлены как есть. На staging
    накопилось заметно больше тестовых данных из нескольких раундов
    регрессионного прогона (заявки #7-14, два акта, тестовая модель, норма
    моточасов, все с префиксами `REGR-TEST-13072026`/`REGR2-TEST-13072026`) —
    это staging, не критично, можно освежить копией с production перед
    следующим большим прогоном, если понадобится чистый лист (см. инструкцию
    в истории чата — снимать СВЕЖИЙ бэкап непосредственно перед копированием,
    не переиспользовать старый файл).
...
## 2026-07-18 — SP-DETAIL-002 (инкремент 2): редизайн карточки заявки — реализован, развёрнут на staging, проверен владельцем

Ветка `sp-detail-002-request-card`, PR #12, HEAD `321d022`. Четыре
упорядоченных коммита; **откат — строго в обратном порядке 4→3→2→1** (откат
одного среднего коммита оставит шаблон обращающимся к контексту, которого
роут уже не отдаёт). Разбивка коммитов — в docs/TASKS.md, раздел
SP-DETAIL-002. Схема, миграции и lifecycle-роуты НЕ тронуты; изменено ровно
три файла.

Что сделано:
1. **Блок «Следующее действие»** вверху карточки — одна крупная
   primary-кнопка, вычисляемая по паре (статус × права текущего
   пользователя): draft/returned_for_revision у владельца → реальный POST
   «Отправить на рассмотрение» / «Исправить и отправить»; submitted + право
   цены + есть неоценённые позиции → «Проставить/подтвердить цену»;
   submitted + право утверждения + нет блокеров `_approval_blockers()` →
   «Утвердить» (primary) + «Отклонить» (вторичная); approved + право выдачи
   → «Выдать со склада». Кнопки цены/утверждения/выдачи не дублируют
   многополевые формы, а скроллят к существующей карточке действия и
   подсвечивают её (~1.5 с). Если действия у пользователя нет — нейтральное
   «⏳ Ожидает: <кто>»; терминальные статусы (rejected/issued) — серая
   строка «действий нет», без кнопки.
2. **Визуальная шкала статуса (степпер)** draft → submitted → approved →
   issued с отметкой текущего шага; returned_for_revision и rejected — не
   отдельные шаги, а боковые чипы (жёлтый «На доработке» / красный
   «Отклонено») при submitted = текущий.
3. **Единая лента событий «История заявки»** — в одну хронологию сведены:
   создание заявки, все смены статуса (`SparePartStatusHistory`, включая
   комментарий рассмотрения), весь аудит цены (`SparePartPriceAudit`:
   указана / подтверждена / отклонена / возвращена, старое→новое) и
   добавление вложений. У каждого события автор и время; имена авторов
   резолвятся в роуте (у истории статусов нет relationship к `User`). Новых
   таблиц не потребовалось — все данные уже существовали.

Удалены как дублирующие: шесть alert-боксов по статусам, карточка «Результат
рассмотрения» (её содержимое теперь видно как события ленты) и отдельная
кнопка «Отправить» внизу страницы.

Файлы: `spare_parts.py` (только `detail()` + добавлен `User` в импорт
моделей), `templates/spare_part_detail.html`,
`static/css/design-system.css` (аддитивно `.vs-nextaction` / `.vs-stepper` /
`.vs-timeline` / `.vs-card.is-flash` на существующих токенах `--vs-*`).

**Ревью архитектора по реальному диффу** (не по отчёту агента): в
`spare_parts.py` три hunk-а (+114/−2), тела всех восьми lifecycle-роутов
(`submit_request`, `approve_request`, `reject_request`, `issue_request`,
`item_price_set`, `item_price_confirm`, `item_price_reject`,
`item_photo_upload`) хеш-идентичны до/после; CSS — один hunk в конце файла,
только добавления; шаблон парсится Jinja без дисбаланса блоков, баланс
`<div>` 110/110, ссылок на удалённую переменную `status_alert_kind` не
осталось.

**Валидация на staging (2026-07-18, самопроверка владельцем под admin):**
проверены все шесть статусов на реальных заявках — #517 draft (владелец
другой → «Ожидает: Оператор отправит заявку»), #524 submitted с
подтверждённой ценой (→ «Утвердить» + «Отклонить»), #521
returned_for_revision (→ «Ожидает: Оператор исправит и отправит» + жёлтый
чип), #525 approved (→ «Выдать со склада»), #518 rejected (→ красный чип,
действий нет), #20 issued (все четыре шага закрыты, лента из 7 событий, акт
списания и ссылки «Открыть»/«PDF» на месте). Дополнительно: на данных
аудита (заявка #20) строки с HTML- и формульными литералами выводятся в
новой ленте экранированно — новый блок не вносит XSS-регрессии.

**НЕ проверено на момент записи** (перенесено на следующий заход, не
блокирует): ветка `kind='submit'` на СОБСТВЕННОМ черновике владельца; ветка
«Проставить/подтвердить цену» на submitted с неоценённой позицией (среди
открытых заявок таких не оказалось); фактическое поведение скролла с
подсветкой; мобильная ширина 380px; переключение интерфейса на узбекский.

Production НЕ трогали (остаётся `ed8ca9c`) — инкремент копится на staging
для единого прод-деплоя бандлом.

## 2026-07-17 — SP-DESK-001 реализован, смержен (PR #11 → main `7d7c1f4`) и рантайм-валидирован на staging

Три независимых коммита (порядок отката — docs/TASKS.md, раздел SP-DESK-001):
(1) общий партиал вкладок `templates/_spare_nav.html` с гейтами по
существующим правам; (2) новый роут `/spare-parts/desk` («Рабочий стол»,
запись в сайдбаре «Запчасти» теперь открывает его; список заявок остаётся на
`spare_parts.index`, запрос index() побайтово не тронут) — очереди по правам
пользователя агрегатными/EXISTS-запросами с org-скоупом; (3) партиал включён
во все 13 шаблонов модуля, разовые кластеры перекрёстных кнопок из hero
убраны (страничные действия «+ Новая заявка» и «⬇ Скачать Excel» сохранены,
на детальных страницах оставлена одна контекстная «назад»-ссылка). Без
миграций, без новых прав, lifecycle-роуты не тронуты. Локально: 65 тестов OK,
рендер-прогон всех 14 страниц модуля RU+UZ по ролям (admin / operator /
viewer / только-цены / без-доступа→403) прошёл. Production НЕ трогали.

**Статус (позже в тот же день, 2026-07-17):** PR #11 смержен в `main`
(`b8cfc3e → 7d7c1f4`, fast-forward), развёрнут на staging (git pull +
рестарт `TransportReportStaging`; миграций/бэкапа не требовалось — схема не
менялась). Рантайм-аудит браузерным QA-агентом (Codex, 6 аккаунтов, ручной
вход, изолированные контексты): **ядро инкремента подтверждено рабочим** —
гейтинг вкладок по правам (пункт B: PASS во всех 6), плитки соответствуют
правам роли, счётчики верны (Ждут цены 8 / Ждут утверждения 55 /
Классификация 3 / Готовы к выдаче 330 / Пора менять 0), активные вкладки,
зелёный акцент `vs-card-accent-success` подтверждён в рантайме, адаптив 380px
без overflow, пустое состояние (operator), узбекский-кириллица без латиницы.
Общий вердикт агента был FAIL, но это **НЕ дефект кода**: (1) 4
специализированных аккаунта заведены с ролью «Оператор» (даёт `can_edit` →
корректно показывает блок «Мои заявки») вместо «Наблюдатель» из тест-матрицы —
расхождение фикстуры, не кода; роли раздаёт владелец; (2) «Пора менять»=0
легитимно (в демо нет техники с подошедшим ТО по Wialon) — сработало слишком
строгое правило «счётчик ≠ 0»; (3) один 404 в консоли — предсуществующий
`/favicon` (в `base_next.html` нет `<link rel="icon">`; новые шаблоны не
ссылаются ни на один статик-файл), не от SP-DESK-001, бэклог. Правок кода не
требуется.

**Дорожная карта заимствования у программ-аналогов** (ТОЛЬКО модуль
«Запчасти», по порядку; отдельный модуль «Наряды» в этом треке НЕ делаем):
1. ✅ Рабочий стол оператора + ролевые экраны — SP-DESK-001 (на staging).
2. **Редизайн карточки заявки** — единая карточка с блоком «Следующее
   действие» + визуальная шкала статуса + единая лента событий (история
   статусов + price-audit + вложения в одну хронологию). ← следующий инкремент.
3. Резерв и доступный остаток — три вида (На складе / Зарезервировано /
   Доступно = склад − резерв), авторезерв при утверждении, освобождение при
   отмене. Трогает склад — аккуратно.
4. Неснижаемый остаток по SKU + авто-дефицит + очередь «Требуется закупка»
   (объединение потребностей + ожидаемые поставки + уведомление о приходе) +
   видимость остатков по холдингу. Наслаивается на п.3.
5. Паспорт техники (единый экран: карточка + моточасы Wialon + открытые
   заявки/выдачи/история ТО + расходы) + отчёт «факт-списание vs норма ТО»
   (усиливает Stage 3).
6. QR на технике → заявка с подставленной техникой/орг (скан → адаптивная
   веб-страница, без натив-приложения). Зависит от п.5.
7. Управленческая аналитика — расходы по технике/модели, частые/дорогие
   детали, оборачиваемость склада, неликвиды.

Дополнения архитектора (вне выводов нейросетей, стандарт для учёта запчастей):
аналоги/замены детали (part↔part) — сильный кандидат; QR/штрих-код на SKU для
склада; история поставщика и цены по SKU. Открытый вопрос (решает владелец):
расширять ли заявку до «заявки-на-ремонт» (метаданные дефекта, без учёта
работ). НЕ берём: отдельный модуль «Наряды», интеграцию/выгрузку в 1С и
бухгалтерию, топливо/ГСМ (это модуль АЗС — отдельный трек), SaaS/мульти-тенант,
no-code конструктор форм.

## 2026-07-15 — SPARE-PARTS-CYCLE-2-3 реализован (ветка claude/session-un5unw, ждёт staging QA)

Девять независимых коммитов (детали и порядок отката — docs/TASKS.md,
раздел SPARE-PARTS-CYCLE-2-3): локализация единиц измерения в PDF/Excel
(RE-SP-010), текст на иконочных кнопках (в т.ч. 💾 цены — пункт из бэклога
полировки закрыт), skip-link/фокус/labels (остаток SP-F-004), лайтбокс
вложений (пункт бэклога закрыт), страница списка актов /spare-parts/acts,
адаптивность 390/1024 + шторка-сайдбар, двуязычные названия деталей
(миграция migrate_spare_parts_name_uz.py — новая additive-колонка
spare_parts.name_uz, переводы владелец заполняет через UI), аудит отказов
доступа (без бэкфила истории — решение владельца), батч-оптимизация N+1
(отчёт 2206→6 запросов, «Пора менять» 2239→4, идентичность результатов
доказана тестами и синтетическим бенчмарком). Полный тестовый прогон:
62 теста OK. Production НЕ трогали.
