...
- Production: commit `0ceea30` (мерж PR #16), развёрнут 2026-07-23.
  ...
  (integrity ok).
- origin/main: commit `0ceea30` (мерж PR #16). Ниже по убыванию: `584ff23`,
...
- Staging: `main` = `0ceea30`, развёрнут 2026-07-23, по коду совпадает
  с продом и с `origin/main`. Бэкап перед инкрементом 4 лежит в
...
- Инкременты трека, все шесть закрыты и лежат в `main`:
...
- Следующий шаг трека — **инкремент 7**, определён владельцем 2026-07-23:
  ...
  не заданы. Прочие кандидаты бэклога трека, не приоритизированы: снятие
  резерва при отклонении утверждённой заявки, история цен на экране SKU,
  массовое утверждение заявок.
