...
Два пункта на момент этой записи **не проверены**: перенос строки на узком
экране и узбекские метки. Оба заведены строкой в `docs/RELEASE_GATE.md`. Первый
особенно важен потому, что перенос вместо прокрутки — отклонение от ТЗ,
авторизованное этим чатом.
