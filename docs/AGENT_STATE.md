...
- origin/main: commit `f956296` (мерж PR #14, инкремент 4). Ниже него по
  убыванию: `fc4b924` (PR #13, инкремент 3), `b6ba298` (доки инкремента 2),
  `ab5c5b2` (PR #12, инкремент 2), `7d7c1f4` (PR #11, инкремент 1).
- Staging: приведён к `origin/main` = `f956296` командой `git reset --hard`
  2026-07-18. Развёрнуты инкременты 1–4. Миграция `SPARE_PARTS_MIN_LEVELS`
  применена, повторный прогон корректно печатает «already applied»; бэкап БД
  снят непосредственно перед деплоем в
  `D:\transport-report-backups\staging_before_SP-MINSTOCK-004\`.
...
- Инкременты трека, все четыре закрыты и лежат в `main`:
...
- Следующий шаг — **инкремент 5: экспорт очереди закупки в Excel** для
  снабжения (решение владельца при постановке инкремента 4: экспорт выносится
  отдельным инкрементом, чтобы не раздувать текущий).
- **Процессный урок этой вехи (важный).** Доки инкремента 3 НЕ были
  закоммичены, хотя промт-передача в новый чат прямо утверждала обратное
  ...
  не после того, как отредактированы.
