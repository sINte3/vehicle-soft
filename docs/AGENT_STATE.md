...
- Дата фиксации: 2026-07-23 (ночь, закрытие трека АЗС — инкременты
  A, A2, A3, B; трек Запчастей — инкремент 7).
- Production: commit `c323745` (мерж PR #23), развёрнут 2026-07-23 в 19:56.
  ...
  Реестр `schema_migrations` — **27 записей**: к прежним 26 добавлена
  `FUEL_MANUAL_EXPENSES_AUTHOR`, применена 2026-07-23 (три nullable-колонки
  `created_by`, `deleted_by`, `deleted_at`; две существовавшие строки
  `fuel_manual_expenses` не тронуты). Бэкапы этого вечера:
  `transport_20260723_175502.db` и `transport_20260723_195637.db`,
  integrity ok у обоих.
...
- **Трек АЗС/топливо (ведётся в отдельном чате).** Закрыты четыре вехи:
  ...
  стороннего топлива по карте с 2026-05-01. Всё на проде и staging,
  проверено на реальных данных против выгрузки 1С. Следующий инкремент —
  `FUEL-RESERVE` (Резерв/Захира). Незакрытый хвост уровня безопасности:
  **`FUEL_API_TOKEN` не ротирован** — владелец 2026-07-23 принял этот риск
  осознанно, решение зафиксировано в `TASKS.md`. Подробности вех — в записях
  от 2026-07-22 и 2026-07-23 ниже.
