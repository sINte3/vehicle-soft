## Текущее состояние — снимок на 2026-07-23 (обновлять при каждой вехе)
...
- Production: commit `402abb7` (мерж PR #24), развёрнут 2026-07-24 в 09:34.
...
- origin/main: мерж PR #19 поверх `8d639da`. Точный хеш мержа брать из
  `git log` — здесь он намеренно не записан, чтобы не разойтись с git.
  Цепочка ниже по убыванию: `25d79e4`, `682cd0c` (коммиты PR #19),
  `8d639da` (доки трека АЗС, параллельный чат), `d5e61a9` (мерж PR #18),
  `d88ae21`, `27cfc77` (коммиты PR #18), `2350033` (мерж PR #17),
  `eacc82f`, `4d44770`, `78ac8f8`, `cc63402` (коммиты PR #17),
  `0ceea30` (мерж PR #16, состояние production). Далее: `584ff23`,
  `7c7e3b3`, `9d05c16` (коммиты инкремента 6), `0a36d4c` (доки прод-деплоя
...
- Staging: `main` = мерж PR #19, развёрнут 2026-07-23. По коду совпадает
  с `origin/main` и опережает production. На staging применён
...
`schema_migrations` остаётся на 26 записях. **На production не развёрнут.**
