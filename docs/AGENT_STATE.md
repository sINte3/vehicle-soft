...
- Активная задача: завершить QA Этапа 2 на staging (см. «Модуль запчастей —
  детальный статус» ниже), затем деплой Этапа 2 на production.
- Незакрытые риски: см. «Модуль запчастей → Этап 2 → открытые вопросы» ниже.
...
- ✅ Открытый вопрос ЗАКРЫТ (2026-07-11): при выдаче позиции без выбранного SKU
  система корректно показала «акт без списания со склада». Подтверждено чтением
  кода (`api_skus_by_part` в `spare_parts.py`, `loadSkus()` в
  `spare_part_form.html`): SKU-дропдаун грузится по `spare_part_id` конкретной
  канонической детали и остаётся скрытым, если для неё не создан ни один SKU —
  это ожидаемое поведение, не баг. В тестовом сценарии SKU был создан для
  «Амортизатора», а заявка — на «Аккумуляторную батарею», у которой SKU не было.
  Осталось пройти сценарий целиком на живых данных: создать SKU именно для
  детали, кладущейся в заявку → выбрать деталь из каталога → убедиться, что
  дропдаун появляется → выдать → подтвердить, что остаток склада реально
  уменьшился.
- ⬜ Не проверено: выдача СО списанием (позиция с SKU, остаток реально
  уменьшился), акт на узбекском на живом сервере, срабатывание правила 3.

**Чтобы считать Этап 2 завершённым:**

1. Пройти сценарий «SKU у детали → заявка → выдача → списание» целиком на
   staging и подтвердить, что остаток уменьшается.
2. Пройти оставшиеся пункты QA (⬜ выше: УЗ-акт, правило 3).
3. Деплой на production: бэкап → `git pull` → проверить шрифты в
