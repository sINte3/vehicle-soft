...
- Дата фиксации: 2026-07-23 (вечер, закрытие инкремента 7).
- Production: commit `0ceea30` (мерж PR #16), развёрнут 2026-07-23.
  Предыдущее состояние — `20eb172`, тег `v1.1-production-2026-07-21`.
  Схема БД с 2026-07-21 не менялась: инкремент 6 — только код и шаблоны,
  миграций нет, реестр `schema_migrations` остаётся на 26 записях
  (`SPARE_PARTS_UNITS`, `SPARE_PARTS_ACTS_PERMISSION`,
  `SPARE_PARTS_SKU_UNIQUENESS`, `SPARE_PARTS_NAME_UZ`,
  `SPARE_PARTS_RESERVATIONS`, `SPARE_PARTS_MIN_LEVELS` применены 2026-07-21).
  Бэкап перед деплоем 2026-07-23: `transport_20260723_092222.db`
  (integrity ok).
  **Инкремент 7 на production НЕ развёрнут** — прод отстаёт от `origin/main`
  на PR #17, #18, #19 и на доки трека АЗС. Деплой возможен только после
  закрытия оставшихся проверок staging (список ниже) и требует прогона
  `add_unit_sqm.py --apply` на проде.
...
- Инкременты трека, все семь закрыты и лежат в `main`:
  ...
  7. **SP-REPORTS-007** (PR #17 + #18 + #19) — разделение экрана отчётов на
     ...
     миграции нет. Развёрнут на staging 2026-07-23; **на production нет**.
...
- **Трек АЗС/топливо (ведётся в отдельном чате).** Закрыта веха
  `FUEL-SYNC-013` — устойчивость синхронизации с Topaz (PR #7, мерж `87863cd`,
  предок текущего HEAD). Код на проде и staging. Незакрытый хвост уровня
  безопасности: **`FUEL_API_TOKEN` не ротирован** после утечки действующего
  прод-токена в переписку. Подробности — в записи от 2026-07-22 ниже.
