## Текущее состояние — снимок на 2026-07-20 (обновлять при каждой вехе)
...
- origin/main: commit `5157b1c` (мерж PR #15, инкремент 5). Ниже него по
  убыванию: `f05c871` (доки инкрементов 3 и 4, закоммичены задним числом),
  `f956296` (PR #14, инкремент 4), `fc4b924` (PR #13, инкремент 3), `b6ba298`
  (доки инкремента 2), `ab5c5b2` (PR #12, инкремент 2), `7d7c1f4` (PR #11,
  инкремент 1).
- Staging: на `main` = `5157b1c`, развёрнуты инкременты 1–5. Инкремент 5
  миграций не требовал (схема не менялась), бэкап БД перед его выкладкой не
  снимался осознанно — фича не пишет в базу ничего. Бэкап перед инкрементом 4
  лежит в `D:\transport-report-backups\staging_before_SP-MINSTOCK-004\`.
