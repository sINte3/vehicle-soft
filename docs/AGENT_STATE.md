...
- Дата фиксации: 2026-07-24 (закрытие трека АЗС целиком — инкременты
  A, A2, A3, B, C; трек Запчастей — инкремент 7).
- Production: **`436d8ae`** — коммит доков трека АЗС поверх `402abb7`
  (мерж PR #24, код развёрнут 2026-07-24 в 09:34). Production, staging и
  `origin/main` сейчас на одном и том же коммите.
  Предыдущее — `c323745` (мерж PR #23), 2026-07-23 в 19:56.
...
- origin/main: **`436d8ae`**. Цепочка ниже по убыванию, оба трека вперемешку —
...
- Staging: `main` = **`436d8ae`**, совпадает с production и с `origin/main`.
  Площадки разошлись только по данным, не по коду. На staging применён
  `add_unit_sqm.py --apply` (единица `kv_metr`, id=6); на production — нет,
