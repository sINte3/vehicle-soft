...
- Дата фиксации: 2026-07-26 (релиз **v1.6** на production — `FUEL-ACL-001`,
  PR #26; мерж `F1.1 · FUEL-NAV-001`, PR #27; ранее — релиз PR #25 и закрытие
  валидации инкремента 7 трека Запчастей).
- Production: **`361661c`**, тег **`v1.6-production-2026-07-26`**. Содержимое
  релиза — мерж **`10a2847`** (PR #26, `FUEL-ACL-001`: доступ к балансовому
  отчёту и его экспорту закрыт модульным правом) плюс пять коммитов доков.
  Деплой 2026-07-26 в 08:10, плановый, по датированному runbook; бэкап
  `transport_20260726_081007.db` снят непосредственно перед деплоем,
  integrity ok; миграций в релизе нет, схема не менялась, откат не
  потребовался. Тег поставлен на **развёрнутый HEAD**, а не на мерж-коммит,
  чтобы `git checkout v1.6-production-2026-07-26` давал прод байт в байт;
  у `v1.5` иначе только потому, что тот деплой был непреднамеренным и HEAD
  успел уйти.
...
- Staging: `main` совпадает с `origin/main` — по коду площадка равна
  ...
  **`361661c`**, тег **`v1.6`**: отстаёт на мерж PR #27 и на всё, что легло в
  `main` после релиза, поэтому строки меню модуля АЗС на проде пока нет. Точное
  отставание — `git log --oneline 361661c..origin/main`. На staging применён
