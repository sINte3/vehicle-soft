...
собран полностью. Дочитано 25.09: `sources_rejected=0`,
`sources_airlines_unmatched=0`, `sources_listener_errors=0`, порядок
715984635 → 716547849. Владелец письменно принял прямой запрос как
безопасный no-op: доказательством отсутствия V4 он не является и площадь не
меняет; такой вылет закрывает решение администратора. Подробности —
`docs/DRONE_AREA_CONTROL_V2_MEGA.md`, §6 и §11 п. 5.
...
## Квалификация V2 на площадке: пройдена 24–25.09.2026

`DRONE-AREA-CONTROL-V2-MEGA` (PR #133). Проверки выполнил владелец на
площадке `C:\transport-report-staging` (служба `TransportReportStaging`, порт
5051, база `instance\transport.db`), ревизия `2013bed`; production не
затрагивался. Строка гейта снята 25.09.2026. Условие за условием —
`docs/DRONE_AREA_CONTROL_V2_MEGA.md`, §9, «Квалификация площадки».

- **Деплой `2013bed`.** HEAD совпал, служба Running, `/login` — 200,
  `/drones/area-control` без входа — 302, рабочее дерево чистое, файлов
  миграций в дельте 0, DJI при деплое не вызывался.
- **Миграция V2.** 25.09 повторный прогон — «Already applied. Nothing to
  do.»; только чтение: миграция в реестре, таблицы `drone_area_cycle_runs` и
  `drone_area_decisions`, триггеры `trg_drone_area_decisions_no_delete` и
  `trg_drone_area_decisions_no_update`, решений 3. Копия перед проверкой —
  `backups\transport_before_area_v2_20260925_121048.db`.
- **Ручное обновление и решение.** 23.09, вылет 715984635: прямой запрос
  дескриптора — HTTP 200, 135 байт, не дескриптор; `sources_rejected=0`,
  `sources_airlines_unmatched=0`, `sources_listener_errors=0`; владелец
  принял прямой запрос как безопасный no-op. Решение «Принять автоматический
  результат»: RAW и принятое — 0,2080 га; видно на экране и в книге.
- **Плановый цикл.** 25.09, окно 24.09, путь по расписанию: обход — 348
  вылетов (новых 167), манифест 9 (кандидатов 3), источники 9/9 с V4, пересчёт
  106 вылетов, итог `SUCCESS`, код 0, журнал `trigger_kind=SCHEDULED`;
  сторож RAW — изменений 0.
- **Экран и книга по-узбекски.** 24.09 00:00–23:59: дерево раскрывается,
  книга совпала с экраном (RAW 119,2262 га, исключено 2,6906, принято
  116,5356, на проверке 0).
- **Backfill.** `--plan` на 208 окон и кусок 01..08.03 на живом чекпойнте
  без очистки — 8 из 8 `DONE`, код 8 (раздел «Исторический backfill»).

**Справочник площадки расходится с production** — это не дефект функции. На
production у дрона № 9 есть `hardware_id` и оба ника, «9 Garden» и
«9 Gagarin»; на площадке `hardware_id` пуст, ника «9 Gagarin» нет. Поэтому за
24.09 на площадке 50 вылетов / 62,9394 га ника «9 Gagarin» не привязаны
к борту («Машина аниқланмаган» 43 / 54,7963 и «борт 010036» 7 / 8,1431), а
на production они у дрона № 9. Сравнивая площадку с production, делать на
это поправку; справочник площадки этой задачей не правится.
...
1. **Гейт.** Таблица «Открытые пункты» в `docs/RELEASE_GATE.md` — изменения,
   которых ещё нет в production, — должна быть пуста. После нормализации
   гейта 25.09.2026 она пуста: строка V2 снята после квалификации площадки, а
   19 прежних строк — post-deploy QA уже работающих изменений; деплой V2 они
   не блокируют (разбор — раздел «Текущий релиз» гейта).
2. **Линия `main`.** Production стоит на `main` `54a875d` (инвентаризация
   25.09.2026, дерево чистое) — это база PR #133; дельта до `main` `3c4e734` —
   только PR #133.
...
## Исторический backfill: квалификация площадки пройдена, полного прохода не было
...
`billable_area_m2` по-прежнему NULL. **Полный backfill не запускался** — ни
на площадке (для квалификации не нужен), ни на production (после релиза). На
площадке выполнены `--plan` и живой кусок 01..08.03 — все 8 окон `DONE`
(ниже, «Квалификация на площадке»); запуск полного прохода и разбор
результата решает владелец.
...
6. **Пустые исторические дни.** Сборщик считает пустое окно ошибкой (код 6,
   ...
   чекпойнте — `empty_window_proof` (контрольный день и вылеты). «Обход» —
   это окно с сутками запаса с обеих сторон: если в нём есть соседние
   вылеты, окно идёт обычным путём и доказательство ему не нужно — пустым
   должен быть именно расширенный обход. Всё остальное **останавливает
   проход с кодом 9**, и попытка окну не списывается:
...
После обновления площадки до `2013bed` (25.09.2026), тем же чекпойнтом — его
**не удаляли и не правили**:

5. Чекпойнт прочитан новым кодом: версия 1, диапазон 01.03..24.09, окно 1
   день; 01.03 — `FAILED`, `attempts=1`, `last_exit=3`, решение — «retry
   after failure». Контроль для 01.03 (проба только на чтение, ничего не
   изменила): разрешён, вылетов окна в базе 0, день 04.03, вылеты `558595607`
   и `558602949`.
6. Кусок `--db instance\transport.db --from 2026-03-01 --to 2026-09-24
   --window-days 1 --max-windows 8`: окна 01..08.03 — **8 из 8 `DONE`**,
   `FAILED` 0, `GAVE_UP` 0, код **8** — остановка по `--max-windows`, окна
   впереди. 01.03 доказан той же сессией (`EMPTY WINDOW PROOF PASSED` в
   `collector.log`). Сторож RAW после куска: изменений 0, пропавших 0,
   `billable_area_m2` не NULL — 0; новые вылеты разрешены.
7. Доказательство понадобилось окну, чей обход с сутками запаса был пуст
   (01.03: 28.02..02.03). Обходы 03.03, 05.03 и 07.03 увидели настоящие
   соседние вылеты (04.03, 04.03 и 08.03) и прошли обычным путём — это и есть
   правило 6, а не недостающая проверка. Прежнее ожидание «пустые 01–03 и
   05–07.03 — с `empty_window_proof`» было неточным. Какие окна куска прошли
   с доказательством — видно по `empty_window_proof` в чекпойнте.

Квалификация на этом закончена. **Полный backfill на площадке не
запускался** и для квалификации не нужен; продолжить его можно той же
командой без `--max-windows`. На production исторический backfill — после
релиза.
