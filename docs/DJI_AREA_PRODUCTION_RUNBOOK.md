...
`/drones/area-control` (раздел «Дроны» → «Контроль площади DJI»), книга —
кнопкой «Excel» на экране, файл `drone_area_control_` + период + `.xlsx`.
Технический экран
...
Арифметика одна: **RAW − исключено = площадь после корректировок**; таблица по
дронам складывается в итог. Строка статуса под числами говорит прямо, все ли
записи периода разрешены.

Реестр показывает цепочку `A → B → C` со ссылками на записи DJI. `A` — базовая
...
Книга: листы «Сводка», «По_дронам», «Корректировки», «Требует_проверки». Книга
всегда содержит полный реестр периода, какой бы фильтр ни стоял на экране.
...
|---|---|---|
| 0 | цикл выполнен | ничего |
| 1 | ошибка аргументов | исправить команду |
| 2 | база не найдена | проверить путь; файл не создаётся |
| 3 | шаг упал, цикл остановлен | строка `STOP: step ... exit N` называет шаг; код 2 шага FLIGHTS — сессия DJI истекла |
| 4 | манифест больше `--stop-above`; **к DJI за источниками не обращались** | см. «Манифест слишком велик» |
| 5 | источники собраны не полностью (код 18 сборщика); пересчёт выполнен | запустить ещё раз: доберёт только недостающее |
...
| цикл возвращает 5 несколько дней подряд | консоль | часть записей стабильно не даёт полного набора; прислать `collector.log` |
...
1. Открыть `/drones/area-control`, фильтр «Подтверждённые корректировки», найти
   запись по дрону и дате.
2. В строке — цепочка `A → B → C`. Открыть ссылки `A` и `C` в DJI: тот же участок,
   у `C` площадь повторяет `A`, а нового обработанного следа нет.
...
человека, поэтому «отключить» сегодня значит «не запускать». Экран при этом
...
**Код.** Экран, книга, endpoint и цикл новых таблиц и колонок не вводят,
миграции в ветке нет. Откат площадки — вернуть прежнюю ревизию блоком ниже;
откат `main` — `git revert` мерж-коммита PR (не `reset`).
...
3. **Расписание.** Отдельная задача планировщика с одной командой
...
## Исторический backfill: описан, не исполняется

Прошлые периоды этой веткой не пересчитываются и задним числом не исправляются.
`UPDATE drone_flights SET area_ha` не выполняется никогда — ни сейчас, ни при
backfill: исправленная площадь всегда живёт рядом с RAW, а не вместо него.

Если владелец решит пройти историю (например, август 2026), порядок такой, и
каждый шаг — отдельное решение:

1. На площадке, не на production. Период — календарный месяц.
2. Манифест за месяц по дням, от старого к новому, с тем же порогом на день.
   Только кандидаты экрана и контрольная выборка; сбора по всему парку нет. У
   DJI телеметрия V4 хранится не вечно: для старых периодов значительная часть
   ответов будет `NO_V4_AT_SOURCE`, такие записи останутся по RAW навсегда.
3. Пересчёт месяца `--apply`, второй прогон `unchanged`, сторож RAW.
4. Экран и книга показывают период сами — отдельной выгрузки не нужно.
5. В расчёты с операторами исторические корректировки попадают только отдельным
   решением владельца: `billable_area_m2` по-прежнему NULL.
