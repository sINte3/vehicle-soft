...
1. Инструмент проходит окно одной командой — сбор и пересчёт на одной машине.
   ...
   (топология production, раздел «Две топологии»). На площадке они разнесены,
   и там backfill этим инструментом не запускается: прошлый день на площадке
   проверяется теми же раздельными шагами суточного цикла (`--from`/`--to` и
