...
|---|---|---|
| 0 | цикл выполнен: итог `SUCCESS`, либо `SUCCESS_WITH_WARNINGS` с одним из предупреждений: `CONTROL_EVIDENCE_MISSING` — без V4 остался только контрольный вылет; `CANDIDATE_NO_V4_AT_SOURCE` — для кандидата DJI V4 не хранит (запись остаётся по RAW как «недостаточно доказательств», не обнуляется); `NO_V4_CHECK_UNAVAILABLE` — после полного сбора не удалось перечитать манифест | при первых двух повтор ничего не даст; при `NO_V4_CHECK_UNAVAILABLE` — повторить прогон. Предупреждения видны в журнале прогонов и в `last_cycle.json`; вылеты «DJI не хранит V4» — ключ `candidates_no_v4_at_source`, решение по ним может принять администратор |
...
VERIFY идёт и после полного сбора (код 0), если манифест называл кандидатов:
«полный» значит, что по каждому получен ответ — V4 либо «у DJI V4 нет», а
которых постигло второе, знает только база. Потери после полного сбора не
судятся; повторный манифест только читает базу и к DJI не ходит. Кандидат,
для которого DJI V4 не хранит, даёт `SUCCESS_WITH_WARNINGS` и
`CANDIDATE_NO_V4_AT_SOURCE` — не провал (повтор ничего не даст) и не чистый
успех (корректировка невозможна). Та же семантика у backfill: окно `DONE` с
этим предупреждением, запись — в списке ручного разбора как
`INSUFFICIENT_EVIDENCE`.

### Дескриптор, которого страница не запросила

Страница записи не всегда запрашивает дескриптор airlines (живой случай
715984635: карточка и маршрут пришли, за 70 с запроса airlines не было вовсе,
ошибок слушателя нет). Раньше такой вылет оставался неполным, сборщик
заканчивал кодом 18, а единственный кандидат делал цикл `FAILED`. Теперь после
ожидания сборщик делает **один прямой GET** дескриптора этого вылета в
контексте уже открытой авторизованной страницы — только для вылета, чья
карточка называет его же и чей маршрут декодирован как он. В журнал попадают
номер вылета и HTTP-статус; адрес, подписанная ссылка и тело — никогда.

- **200 с дескриптором** — как собственный ответ страницы: без ссылки V4 —
  `NO_V4_URL`, со ссылкой — V4 скачивается по ней один раз и проверяется, что
  это объект именно этого вылета.
- **404** — «у DJI нет дескриптора, значит нет V4», но **только** после одного
  контрольного запроса на прогон: тот же прямой GET для вылета, у которого
  дескриптор точно есть (страница получила его в этом прогоне, либо его V4 уже
  в очереди), обязан вернуть 200 с дескриптором. Тогда 404 сохраняется как
  ревизия airlines схемы `airlines-descriptor-absent-1`, приёмник её
  перепроверяет, вылет получает `NO_V4_URL_AT_SOURCE`, манифест перестаёт его
  звать и называет в `no_v4_at_source`, цикл даёт `SUCCESS_WITH_WARNINGS` с
  `CANDIDATE_NO_V4_AT_SOURCE`. Площадь не обнуляется: запись остаётся по RAW.
- **Всё остальное** — 3xx, 401, 403, 5xx, сбой сети, 2xx не-дескриптор — **не**
  отсутствие: вылет неполный, код 18, как раньше.

Контрольный запрос — то, что делает 404 доказательством. Если DJI требует
для этого адреса подпись своего WASM (см. README сборщика), контроль не
пройдёт, все 404 останутся неподтверждёнными, и прогон закончится кодом 18
...
или `-`, если 404 не было и подтверждать было нечего). Как читать:

| Сводка | Что это | Что делать |
|---|---|---|
| `sources_descriptor_absent>=1`, `sources_descriptor_control=OK` | DJI ответил 404 на дескриптор вылета, контроль прошёл; отсутствие записано | ничего: цикл — `SUCCESS_WITH_WARNINGS`, вылет в `candidates_no_v4_at_source` |
| `sources_descriptor_unconfirmed>=1`, `sources_descriptor_control=FAILED` | 404 есть, но прямой запрос не прошёл контроль (или контролировать было не по чему) | повторить прогон; если повторяется — прямой запрос к DJI не работает, прислать журнал сборщика; вылет остаётся «ожидает доказательства» |
| `sources_descriptor_refused>=1` | прямой запрос получил 3xx/401/403/5xx/сбой/не-дескриптор | отсутствием это не считается никогда. Если остальные вылеты прогона собраны полностью (сессия жива) — повтор, скорее всего, ничего не даст: запись остаётся по RAW «ожидает доказательства», решение — за администратором. Если провалились и они — проверить сессию DJI и повторить |

Живьём 24.09.2026 (площадка, `7e6d00d`, вылет 715984635) случилась именно
третья строка: HTTP 200, 135 байт, не дескриптор; `requests=1`,
`refused=1`, `control=-`, сборщик 18, цикл 5. Другой вылет того же прогона
собран полностью. Подробности и что из этого следует —
`docs/DRONE_AREA_CONTROL_V2_MEGA.md`, §6.
...
| панель «Обновить данные DJI»: «Успешно, с предупреждениями» | экран | фраза рядом называет причину: без V4 остался только контрольный вылет — ничего не делать; DJI не хранит V4 для кандидатов — записи остаются по RAW, решение за администратором; повторный манифест не прочитан — повторить прогон |
...
3. **Миграция V2.** `migrate_drone_area_control_v2_001.py` при остановленной
   службе и снятой копии. Обе таблицы приложение создаёт и само
   (`db.create_all()` при старте), поэтому решения и журнал обновлений
   заработают и без неё — но без триггеров, запрещающих UPDATE/DELETE истории
   решений, и без записи в реестре миграций. Пока триггеров нет, экран
   «Контроль площади DJI» говорит это администратору словами. Миграция
   идемпотентна: после `create_all` она добавляет недостающее и
   регистрируется.
...
## Исторический backfill: квалификация начата, полного прохода не было
...
`billable_area_m2` по-прежнему NULL. **Полный backfill не проходил ни разу.**
На площадке 24.09.2026 выполнены `--plan` и одна попытка первого окна (ниже,
«Квалификация на площадке»); запуск и разбор результата решает владелец.

Правила:

1. Инструмент проходит окно одной командой — сбор и пересчёт на одной машине.
   Поэтому он годится только там, где сборщик и база стоят на одном хосте
   (топология production, раздел «Две топологии»). Квалификация 24.09.2026
   шла с хоста площадки `C:\transport-report-staging`, где есть и сборщик
   (`drone_collector\data\area_backfill\checkpoint.json`), и база. Где они
   разнесены, backfill этим инструментом не запускается: прошлый день
   проверяется раздельными шагами суточного цикла (`--from`/`--to` и
   `--skip-recalc` на рабочей машине, затем `--recalc-only` на сервере).
   Окно — от одного до семи дней, никогда не пересекает границу месяца; идут
   от старого к новому.
...
3. Остановить можно в любой момент (Ctrl+C, `--max-windows N`); следующий запуск
   ...
   диапазоном, другой длиной окна или от другой базы (`--db`) не
   смешивается — инструмент отказывает с кодом 1 и файл не трогает.
...
5. Кандидат без V4 делает окно `FAILED` (код цикла 5); промах только
   контрольной выборки — `DONE` с предупреждением. Кандидат, для которого DJI
   V4 не хранит, — `DONE` с предупреждением `CANDIDATE_NO_V4_AT_SOURCE`
   (как у суточного цикла); запись попадает в `backfill_unresolved.csv` как
   `INSUFFICIENT_EVIDENCE`, а контрольная точка окна хранит список
   `candidates_no_v4_at_source`.
6. **Пустые исторические дни.** Сборщик считает пустое окно ошибкой (код 6,
   Guard A): сессия в чужом регионе возвращает ноль строк без всякой ошибки.
   Глобально этот сторож не снимается, и `DJI_ALLOW_EMPTY_WINDOW` backfill не
   использует. Вместо этого окну, о котором база не знает ни одного вылета,
   инструмент передаёт **контроль**: ближайший отчётный день с вылетами наших
   бортов (не ближе чем через сутки от обхода) и до пяти их идентификаторов
   DJI. Если обход окна пуст, та же сессия, в том же процессе, сразу обходит
   контрольный день с сутками запаса. Нашла хотя бы один из названных
   вылетов — окно пусто и это доказано: код сборщика 25, окно `DONE`, в
   чекпойнте — `empty_window_proof` (контрольный день и вылеты). Всё
   остальное **останавливает проход с кодом 9**, и попытка окну не
   списывается:
   - контрольный обход пуст, неполон или в нём нет ни одного из названных
     вылетов (чужой регион или аккаунт тоже может вернуть непустой список —
     поэтому нужен точный id);
   - база знает вылеты окна, а DJI не вернул ни одного — противоречие, не
     пустой день; доказательство такому окну не даётся вовсе;
   - сессия истекла (код сборщика 2) или регион не тот (7).

   Причина остановки — в `stops` чекпойнта (окно, код сборщика, контроль,
   сколько вылетов окна знает база). «Регион подтверждён» не хранится нигде:
   каждое пустое окно доказывает своя сессия заново — после паузы, после
   пересохранения сессии, после продолжения через дни. Отсутствие вылетов в
   базе само ничего не доказывает, оно лишь разрешает попытку. Занятый
   сборщик (24) — как занятый цикл: код 7, окно `BUSY`, попытка не списана.
...
| `7` | остановлен: блокировку цикла или сборщика держит другой сбор; окно `BUSY`, попытка не списана; запустить позже той же командой |
| `8` | остановлен по `--max-windows`, окна ещё остались; запустить ту же команду снова |
| `9` | остановлен: сессия DJI не доказала, что видит наши вылеты (пустое окно без доказательства, пустой ответ там, где база знает вылеты, сессия, регион); окно не отмечено, попытка не списана; причина — `stops` в чекпойнте и `collector.log`; исправить сессию и запустить ту же команду |
...
### Квалификация на площадке, 24–25.09.2026

Что было живьём (площадка, код ветки до правки пустого окна):

1. `--plan --from 2026-03-01 --to 2026-09-24` — 208 однодневных окон, код 0,
   ничего не записано, к DJI не обращался.
2. Первая попытка окна `2026-03-01`: обход вылетов 28.02..02.03 вернул **ноль
   вылетов**, сборщик — `EMPTY WINDOW`, код 6; цикл — код 3, `STEP_FAILED` на
   `FLIGHTS`; чекпойнт — окно `FAILED`, `attempts=1`, `last_exit=3`.
3. База площадки (только чтение): в 2026-03 — 1398 вылетов, но 01, 02, 03,
   05, 06, 07.03 — ноль, 04.03 — два, 08.03 — 31; первый вылет диапазона
   `558595607`, 2026-03-04 06:29:50 UTC. По месяцам: 03 — 1398, 04 — 6402,
   05 — 2537, 06 — 168, 07 — 0, 08 — 401, 09 — 5849; всего 16 755.
4. Живой положительный контроль (`--from 2026-03-03 --to 2026-03-05 --kind
   backfill --dry-run`): период проверен точно, DJI вернул ровно **2**
   вылета, код 0 — те самые два вылета 04.03 из базы.

Вывод: сессия рабочая и видит наш набор данных, 01.03 действительно пуст, а
прежний backfill законный пустой день превращал в `FAILED`, а после трёх
попыток — в `GAVE_UP`. Для этого понадобилось доказательство пустого окна
(правило 6). **Полный backfill не проходил.**

Что повторить на площадке после обновления кода (с хоста площадки, из
`C:\transport-report-staging`, интерпретатором venv сборщика; `--db` — та же
база, к которой привязан чекпойнт 24.09: другая база даст отказ с кодом 1 и
её имя в сообщении). Чекпойнт **не удалять и не править**: окно
`2026-03-01` в нём `FAILED`, `attempts=1`, и та же команда просто повторит
его второй попыткой.

```text
drone_collector\.venv\Scripts\python.exe tools\dji_area_backfill.py --plan --from 2026-03-01 --to 2026-09-24
drone_collector\.venv\Scripts\python.exe tools\dji_area_backfill.py --db instance\transport.db --from 2026-03-01 --to 2026-09-24 --window-days 1 --max-windows 8
```

Восемь окон 01..08.03 проверяют обе ветки: пустые 01–03 и 05–07.03 должны
получить строку `empty-window control: N known flight(s) of our drones on`
(ближайший день с вылетами наших бортов вне обхода; для 01.03 — 2026-03-04),
`EMPTY WINDOW PROOF PASSED` в `collector.log` и `DONE` с
`empty_window_proof`; 04 и 08.03 — обычный путь без контроля. Ожидаемый код —
8 (окна ещё остались). Код 9 — не сбой инструмента, а остановка по сессии:
прислать строки `STOP:` и `EMPTY WINDOW PROOF FAILED` из `collector.log`.
