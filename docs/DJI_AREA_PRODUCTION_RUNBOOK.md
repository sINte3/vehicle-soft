...
1. **Гейт.** `docs/RELEASE_GATE.md` должен опустеть — это условие всего
   проекта. Строка V2 снята 25.09.2026 после квалификации площадки; в таблице
   остаются строки других инкрементов трека.
